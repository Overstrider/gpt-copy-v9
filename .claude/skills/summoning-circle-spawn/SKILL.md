...
description: "Reference skill: summoning-circle-spawn contract for the Loldinis pipeline. Documents the type matrix (Â§8.1), spawn prompt template (Â§8.2), and return contract (Â§8.3). Other skills and phase commands reference this so isolation rules stay consistent. Does NOT itself spawn subagents â€” Read-only reference."
---
...
| Fast glob/grep (one-shot) | (do NOT spawn â€” parent calls Grep/Glob directly) | â€” | Spawning for one search wastes tokens. |
...
**Rule:** never spawn a subagent you could replace with a direct tool call (Read / Grep / Glob / Bash). The ROI hook (Â§8.4 below) logs a warning if spawn input > 10x output tokens.

## Â§8.2 â€” Spawn prompt template
...
max_thinking_tokens: {per Â§12 table}
model: {per Â§2 table}
```

**Never** let a spawn prompt carry the full codebase or full arcplan â€” only the narrow slice the subagent needs. Handoff files (Â§6) exist for this purpose.

## Â§8.3 â€” Return contract
...
> Subagent spawning: follow `summoning-circle-spawn/SKILL.md` â€” type matrix, spawn template, return contract, ROI hook.
