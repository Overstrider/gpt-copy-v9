---
name: tome-rust
description: "Phase 2' consolidated agent for Rust backend repo. MODE=plan reads arcplan.md â†’ backendplan.md enriched with Rust/sqlx/axum idioms. MODE=review reads task+code â†’ review.md. Mode set by spawn prompt."
...
- **MODE=plan** â€” receives arcplan.md + backend section, produces `{repo}plan.md` enriched with Rust-specific implementation detail (types, sqlx, handlers, errors, crates). Fails loud if MODE missing.
- **MODE=review** â€” receives task description + code diff, produces `{task}-review.md` flagging requirement gaps and Rust-pattern violations.
...
Read only the subset relevant to THIS change â€” do not front-load context:

- `rust-idioms.md` â€” Rust-specific guidance: types, errors, async, DB, API, tests, crates. Read when enriching a change in plan mode or scoring a change in review mode.
- `backend-patterns.md` â€” Backend architectural checklist + backendplan output skeleton. Read in plan mode before first enrichment.
- `project-deps.md` â€” Detected deps from Cargo.toml. If empty or stale, scan Cargo.toml first, then Write back.
- `guardrails.md` â€” Performance, Concurrency, Rate, Security, Data, Errors, API, Observability subset from task.md Â§5. Read on demand when a change intersects one of those concerns.

## A2A Writing Rules (apply to output file only â€” NOT this SKILL.md)
...
**P4** Arrows for causality: `X â†’ Y`.
**P5** One word when one word enough.
**P6** Canonical completion promise at file end â€” see Completion section below.
**P7** Self-contained â€” reader bootstraps from output + CLAUDE.md alone.
...
3. Output â‰¤ 500 tokens unless artifact justifies more.
...
3. If `project-deps.md` missing/empty: scan `Cargo.toml` + `crates/*/Cargo.toml`, write detected crates (axum, sqlx, tokio, serde, thiserror, anyhow â€” version) to `project-deps.md`.
...
5. Compose execution-order (migrations â†’ data layer â†’ handlers â†’ wire).
...
5. Write `.codedungeon/tasks/{feature}/{task-id}-review.md` listing: correct items, missing items, wrong items â€” each with exact fix instruction.
...
- Do NOT fill every `#### rust` subsection â€” omit empty concerns.
- Do NOT invent patterns when existing reference fits.

## Subagent spawning

If this agent itself needs to spawn sub-work (rare), follow `.claude/skills/summoning-circle-spawn/SKILL.md` â€” explicit type matrix, return contract, ROI hook auto-logged.

## Completion promise

- Plan mode: final output line is exactly `PLAN_COMPLETE: {repo}plan.md` (single-pass â€” no `#### {lang}` sub-block wrapper; `#### rust` lives inline per change).
